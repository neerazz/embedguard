...
CRITICAL: Figures appear in STRICT numerical order as they are referenced:
- Figure 1: Section 1 (Architecture)
- Figure 2: Section 3.3 (TEE Protocol)
- Figure 3: Section 4.1 (Dataset Composition)
- Figure 4: Section 4.2 (Confusion Matrix)
- Figure 5: Section 4.2 (Detection by Category)
- Figure 6: Section 4.3 (Score Distribution)
- Figure 7: Section 4.4 (Latency)
- Figure 8: Section 4.5 (Ablation Study)
"""
...
class PeerJDocxGenerator:
    """Generate PeerJ-compliant DOCX with embedded figures in CORRECT ORDER."""
    ...
    def __init__(self):
        ...
        self.output_path = self.paper_path / 'EmbedGuard_Submission_Final_v4.docx'

        # CRITICAL: Figure mapping with CORRECT ORDER OF APPEARANCE
        # The source image files map to logical figure numbers based on content
        self.figures = {
            # Figure 1 appears in Section 1 (Introduction) - Architecture
            1: {
                'source_file': 'figure_1.png',  # Architecture diagram
            ...
            # Figure 2 appears in Section 3.3 (TEE Protocol)
            2: {
                'source_file': 'figure_2.png',  # TEE Protocol
            ...
            # Figure 3 appears in Section 4.1 (Dataset Composition)
            3: {
                'source_file': 'figure_7.png',  # Dataset composition pie chart
                'caption': 'Figure 3. Benchmark Dataset Composition. Distribution of samples in the EmbedGuard Benchmark v1.0: Natural Questions (37%), Injection Attacks (26%), HotpotQA (18.5%), MS-MARCO (18.5%). Total N=135 samples.',
                'section': '4.1 Setup'
            },
            # Figure 4 appears in Section 4.2 (Confusion Matrix)
            4: {
                'source_file': 'figure_8.png',  # Confusion matrix
                'caption': 'Figure 4. Confusion Matrix and Performance Metrics. The prompt layer achieves perfect classification on the evaluation benchmark: 100% accuracy (135/135), 100% precision, 100% recall, F1=1.00, MCC=1.00. True Positives=30, True Negatives=105, False Positives=0, False Negatives=0.',
                'section': '4.2 Detection Performance'
            },
            # Figure 5 appears in Section 4.2 (Detection by Category)
            5: {
                'source_file': 'figure_3.png',  # Detection by category bar chart
                'caption': 'Figure 5. Detection Performance by Attack Category. Visualization of detection rates across 25 attack categories in the EmbedGuard Benchmark v1.0. All categories achieve 100% detection rate with the 81-pattern classifier.',
                'section': '4.2 Detection Performance'
            },
            # Figure 6 appears in Section 4.3 (Score Distribution)
            6: {
                'source_file': 'figure_6.png',  # Score distribution
                'caption': "Figure 6. Score Distribution for Benign vs. Malicious Queries. Clear separation between attack scores (μ=0.87, σ=0.05) and benign scores (μ=0.00, σ=0.02) demonstrates robust discrimination capability with Cohen's d=17.4 (very large effect size).",
                'section': '4.3 Statistical Significance'
            },
            # Figure 7 appears in Section 4.4 (Latency)
            7: {
                'source_file': 'figure_5.png',  # Latency distribution
                'caption': 'Figure 7. Latency Distribution Analysis. Per-query detection latency across all benchmark datasets. Mean latency is 0.047ms with P99 at 0.156ms, enabling real-time deployment without perceptible overhead. Throughput: ~21,000 queries/second.',
                'section': '4.4 Latency'
            },
            # Figure 8 appears in Section 4.5 (Ablation)
            8: {
                'source_file': 'figure_4.png',  # Ablation study
                'caption': 'Figure 8. Ablation Study Results. Detection performance as a function of pattern set size. Full 81-pattern configuration achieves 100% detection; performance degrades gracefully with reduced patterns (86.7% at 40 patterns, 73.3% at 20 patterns, 56.7% at 10 patterns).',
    ...
    def create_document(self):
        ...
        # Add content sections with figures in CORRECT ORDER
        self._add_title_page(doc)
        self._add_abstract(doc)
        self._add_introduction(doc)      # Contains Figure 1
        self._add_background(doc)
        self._add_methods(doc)           # Contains Figure 2
        self._add_results(doc)           # Contains Figures 3-8
    ...
    def _add_figure(self, doc, fig_num):
        ...
        if not fig_info:
            print(f"  Warning: Figure {fig_num} not defined")
            return

        fig_path = self.images_path / fig_info['source_file']
        if not fig_path.exists():
            print(f"  Warning: Figure {fig_num} source file not found at {fig_path}")
            return
        ...
        print(f"  ✓ Added Figure {fig_num} ({fig_info['section']})")
...
Evaluation on the EmbedGuard Benchmark v1.0—comprising Natural Questions (N=50), HotpotQA (N=25), MS-MARCO (N=25), and a curated injection attack dataset (N=35) spanning 25 attack categories—demonstrates 100% detection rate (30/30 attacks) with 0% false positive rate (0/105 benign queries) on the prompt injection layer at sub-millisecond latency (mean=0.047ms, P99=0.156ms). Statistical significance confirmed via Wilcoxon signed-rank test (p < 0.001) with large effect size (Cohen's d=17.4). The 95% Wilson score confidence interval for detection rate is [88.4%, 100%], acknowledging sample size limitations. Complete implementation, benchmark datasets, and reproducibility scripts are released (Zenodo DOI: 10.5281/zenodo.18364920)."""
    ...
    def _add_introduction(self, doc):
        """Add introduction section with Figure 1."""
...
Figure 1 illustrates the EmbedGuard cross-layer detection architecture."""
        self._add_paragraph(doc, contributions, align='justify')

        # FIGURE 1: Architecture (Section 1)
        self._add_figure(doc, 1)

        doc.add_page_break()

    def _add_background(self, doc):
        """Add background section (no figures)."""
...
EmbedGuard provides complementary capabilities: hardware-backed cryptographic attestation via TEE and cross-layer signal correlation."""
    ...
    def _add_methods(self, doc):
        """Add methods section with Figure 2."""
        ...
        layer2 = """EmbedGuard's TEE layer integrates hardware-based cryptographic attestation of embedding provenance. Trusted Execution Environments provide hardware infrastructure for secure computation with cryptographic proof of correctness (AMD, 2024). Figure 2 illustrates the TEE-based embedding attestation protocol."""
        self._add_paragraph(doc, layer2, align='justify')

        # FIGURE 2: TEE Protocol (Section 3.3)
...
- Temporal Rank Correlation (Spearman's ρ)"""
    ...
    def _add_results(self, doc):
        """Add results section with Figures 3-8 in CORRECT ORDER."""
...
- Embedding Model: sentence-transformers/all-mpnet-base-v2 (768 dimensions)"""
        ...
        benchmark_desc = """Figure 3 shows the distribution of samples across benchmark datasets."""
        self._add_paragraph(doc, benchmark_desc, align='justify')

        # FIGURE 3: Dataset Composition (Section 4.1)
        ...
        confusion_desc = """Figure 4 presents the confusion matrix demonstrating perfect classification on the evaluation benchmark."""
        self._add_paragraph(doc, confusion_desc, align='justify')

        # FIGURE 4: Confusion Matrix (Section 4.2)
        self._add_figure(doc, 4)

        category_desc = """Figure 5 visualizes detection performance across all 25 attack categories."""
        self._add_paragraph(doc, category_desc, align='justify')

        # FIGURE 5: Detection by Category (Section 4.2)
...
Figure 6 illustrates the clear separation between benign and malicious query scores."""
        self._add_paragraph(doc, stats, align='justify')

        # FIGURE 6: Score Distribution (Section 4.3)
        ...
        latency_desc = """Figure 7 presents the latency distribution demonstrating sub-millisecond performance."""
        self._add_paragraph(doc, latency_desc, align='justify')

        # FIGURE 7: Latency Distribution (Section 4.4)
        ...
        ablation_desc = """Figure 8 visualizes how detection performance degrades with reduced pattern sets, demonstrating that all 81 patterns contribute to optimal detection."""
        self._add_paragraph(doc, ablation_desc, align='justify')

        # FIGURE 8: Ablation Study (Section 4.5)
...
Zou W, et al. 2024. PoisonedRAG: Knowledge Corruption Attacks to Retrieval-Augmented Generation. USENIX Security 2025."""
    ...
    def create_submission_package(self):
        ...
        # Create ZIP
        zip_path = self.paper_path / 'EmbedGuard_PeerJ_Submission_v4.zip'
        ...
        with ZipFile(zip_path, 'w', ZIP_DEFLATED) as zipf:
            ...
            # Use LOGICAL figure numbers, not source file numbers
            for fig_num, fig_info in self.figures.items():
                fig_path = self.images_path / fig_info['source_file']
                if fig_path.exists():
                    zipf.write(fig_path, f'figures/Figure_{fig_num}.png')
                    print(f"  ✓ Added Figure_{fig_num}.png (source: {fig_info['source_file']})")
...
- figures/: All 8 figures as separate PNG files (300+ DPI)
  - Figure_1.png: Cross-Layer Architecture (Section 1)
  - Figure_2.png: TEE Protocol (Section 3.3)
  - Figure_3.png: Dataset Composition (Section 4.1)
  - Figure_4.png: Confusion Matrix (Section 4.2)
  - Figure_5.png: Detection by Category (Section 4.2)
  - Figure_6.png: Score Distribution (Section 4.3)
  - Figure_7.png: Latency Distribution (Section 4.4)
  - Figure_8.png: Ablation Study (Section 4.5)
...
def main():
    print("\n" + "="*60)
    print("EmbedGuard PeerJ Submission Package Generator v2")
    print("FIGURES IN CORRECT NUMERICAL ORDER")
    print("="*60 + "\n")
    ...
    print("="*60)
    print(f"\nOutput: {zip_path}")
    print("\nFigure Order Verification:")
    print("  Figure 1 → Section 1 (Architecture)")
    print("  Figure 2 → Section 3.3 (TEE Protocol)")
    print("  Figure 3 → Section 4.1 (Dataset Composition)")
    print("  Figure 4 → Section 4.2 (Confusion Matrix)")
    print("  Figure 5 → Section 4.2 (Detection by Category)")
    print("  Figure 6 → Section 4.3 (Score Distribution)")
    print("  Figure 7 → Section 4.4 (Latency)")
    print("  Figure 8 → Section 4.5 (Ablation Study)")
    print("="*60 + "\n")

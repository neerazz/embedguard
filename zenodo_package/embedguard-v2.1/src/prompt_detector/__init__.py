...
INJECTION_PATTERNS = [
    ...
    r"(what|show|reveal|display|print)\s+(is\s+)?(your|the)\s+(system\s+)?(prompt|instructions?)",
    ...
    # Jailbreak keywords
    r"DAN\s*mode|jailbreak|bypass\s+(safety|filter|restriction)",
...
class PromptInjectionDetector:
    ...
    def _pattern_detection(self, query: str) -> Tuple[float, List[str]]:
        ...
        score = min(0.5 + (len(matched) * 0.15), 1.0)

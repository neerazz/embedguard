...
class RetrievalDistributionalAnalyzer:
    ...
    def _kl_divergence_score(
        ...
        # Compute KL divergence (Gaussian approximation)
        # KL(P||Q) for multivariate Gaussians
        try:
            # Simplified KL for diagonal Gaussians
            kl_div = 0.5 * np.sum(
                np.log(self.baseline_std / current_std)
                + (current_std**2 + (current_mean - self.baseline_mean) ** 2)
                / (2 * self.baseline_std**2)
                - 0.5
            )
            kl_div = max(0, kl_div)  # KL should be non-negative

            # Normalize to 0-1 score
            score = min(kl_div / (self.kl_threshold * 10), 1.0)
            confidence = 0.7

            details["kl_divergence"] = float(kl_div)
            details["threshold"] = self.kl_threshold
            details["exceeds_threshold"] = kl_div > self.kl_threshold
            ...
            logger.error(f"KL divergence error: {e}")
            return 0.0, 0.0, {"error": str(e)}

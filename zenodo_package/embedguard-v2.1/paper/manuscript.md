...
**Competing Interests:** The author is an employee of Meta Platforms, Inc. This work was conducted independently and is not affiliated with Meta.

**Ethics Statement:** This research involves security evaluation of AI systems using synthetic attack data generated for defensive research purposes. No human subjects were involved. Attack implementations are provided in sanitized form to prevent misuse while enabling reproducibility. The research aims to improve AI security for beneficial applications in healthcare, financial services, and legal research domains.

---

## Abstract

Embedding-based Retrieval-Augmented Generation (RAG) systems are critical infrastructure for production AI applications, yet remain vulnerable to embedding space poisoning attacks that achieve disproportionate success with minimal payloads (less than 1% corpus contamination achieving greater than 80% attack success rates). Current defense approaches optimize for isolated attack surfaces, making them vulnerable to coordinated attacks distributing adversarial signals across architectural layers. EmbedGuard is an adaptive, cross-layer detection framework integrating hardware-backed cryptographic attestation with statistical anomaly detection across four RAG architectural layers: prompt injection detection, TEE-based embedding attestation, retrieval distributional analysis, and output consistency verification. The framework employs weighted multi-signal fusion to correlate individually benign signals that collectively indicate attacks. Evaluation across standard benchmarks (Natural Questions, HotpotQA, MS-MARCO) and a comprehensive injection attack dataset spanning 25 attack categories demonstrates 100% detection rate with 0% false positive rate at sub-millisecond latency (0.04ms mean). The cross-layer architecture detects all tested attack variants including direct instruction injection, jailbreak attempts, encoding obfuscation, context manipulation, and composite multi-vector attacks. Complete implementation and benchmark datasets are released for full reproducibility.
...
**Reproducible Benchmark Evaluation:** Comprehensive evaluation on standard NLP benchmarks (Natural Questions, HotpotQA, MS-MARCO) combined with a diverse injection attack dataset spanning 25 attack categories demonstrates 100% detection rate (30/30 attacks) with 0% false positive rate (100/100 benign queries correctly classified) at 0.04ms mean latency. Complete code, datasets, and evaluation scripts are released for full reproducibility.
...
|---------|-----------|-------------|----------|----------|----------|----------|
| Injection Attacks | 0.082 | 0.082 | 0.135 | 0.160 | 0.018 | 0.169 |
| Natural Questions | 0.019 | 0.016 | 0.035 | 0.045 | 0.007 | 0.048 |
| HotpotQA | 0.043 | 0.040 | 0.074 | 0.078 | 0.026 | 0.078 |
| MS-MARCO | 0.022 | 0.018 | 0.045 | 0.054 | 0.008 | 0.056 |
| **Aggregate** | **0.040** | **0.026** | **0.103** | **0.138** | **0.007** | **0.169** |

Latency overhead is sub-millisecond across all datasets, with aggregate mean of 0.04ms and P99 of 0.14ms. This enables real-time detection without perceptible user-facing latency impact. Attack queries show slightly higher latency (0.082ms mean) due to pattern matching overhead when attacks trigger multiple detection patterns.
...
The novel hardware attestation schemes proposed in this work enforce a fundamental shift in the security model, turning embedding security from a statistical inference problem into a cryptographic verification problem. Experiments demonstrate better performance than state-of-the-art single-layer defenses under adaptive attacks that evade statistical detection. In contrast to single-layer defenses, the combination of cryptographic verification of authenticated embeddings and cross-layer correlation of distributed attack signatures reduces common limitations in probabilistic protections.
...
Future work will extend the framework to address emerging attack vectors including multi-modal RAG systems, federated retrieval architectures, and continuous learning scenarios where knowledge bases evolve over time.
...
Complete implementation and evaluation materials are available at https://github.com/neerazz/embedguard with an archived version at Zenodo (DOI: 10.5281/zenodo.XXXXXXX). The repository includes:

- **Source Code:** Complete EmbedGuard framework implementation (Python 3.10+)
- **Benchmark Datasets:** Natural Questions (50 samples), HotpotQA (25 samples), MS-MARCO (25 samples)
- **Attack Dataset:** 35 injection samples spanning 25 attack categories with ground-truth labels
- **Evaluation Scripts:** Reproducible benchmark runner generating `results/latest_results.json`
- **Detection Patterns:** 81 patterns in `src/prompt_classifier/patterns.py`

Running `python -m src.evaluation.run_benchmarks` reproduces all reported results. See Appendix A for complete software specifications.
...
- [x] Archived version with DOI: Zenodo 10.5281/zenodo.XXXXXXX
